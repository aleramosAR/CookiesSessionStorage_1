...
async function formLogin(e) {
  ...
  if (inputNombre.value == '') {
    alert('Por favor completa el formulario para ingresar.')
  } else {
    try {
			const res = await fetch(`${URL_BASE}/api/auth/login?usuario=${inputNombre.value}`, {
        method: 'GET',
        headers: {
      ...
      if (res.status == 200) {
        ...
        alert("Hubo un problema al ingresar.")
      }
		} catch (err) {
      alert(err)
		}
...
if(window.location.href.split('/').pop() == 'index') {
